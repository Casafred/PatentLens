...
(function () {
  ...
  function renderClaimsBilingual(record, config) {
    ...
    var originalHtml = renderClaimsHtml(claims, config, record.claimsAnnotations);
  ...
  function renderSourcePanel(record, config) {
    ...
    if (moduleEnabled(config, "S4")) {
      ...
      if (record.claimsTranslation) {
        ...
        html += renderClaimsHtml(record.claims, config, record.claimsAnnotations);
