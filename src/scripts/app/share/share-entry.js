...
(function () {
  ...
  function renderModules(container, project) {
    ...
    var hint = makeElement("p", "share-module-hint", "必要模块不能关闭；配置会自动保存。基础信息从来源提取并可人工校核；加工信息可AI抽取或手工录入，支持自定义字段与提示词。");
    container.appendChild(hint);
    var categories = registry.listByCategory();
    var categoryLabels = { basic: "基础信息（从来源提取 + 人工校核）", processed: "加工信息（AI 抽取 / 手工录入）" };
    Object.keys(categories).forEach(function(catKey) {
      if (!categories[catKey] || !categories[catKey].length) return;
